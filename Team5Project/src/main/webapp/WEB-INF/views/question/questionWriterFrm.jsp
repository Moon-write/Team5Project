<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
</head>
<body>
	<%@include file="/WEB-INF/views/common/header.jsp" %>
	<div class="page-content">
		<div class="page-title">질문 작성</div>
		<form action="/questionWrite.do" method="post">
			<tr class="tr-1">
				<th class="td-1">제목</th>
				<td colspan="3">
					<input type="text" name="noticeTitle" class="input-form">
				</td>
			</tr>
			<tr class="tr-1">
				<th class="td-2">작성자</th>
				<td>
					<input type="hidden" name="notceWriter" value="<%=m.getMemberId() %>">
					<%=m.getMemberId() %>
				</td>
				<th class="td-2">썸머노트!!</th>
				<td><input type="file" name="file"></td>
			</tr>
			<tr class="tr-1">
				<td colspan="4"><textarea name="noticeContent" class="input-form"></textarea></td>
			</tr>
			<tr>
				<td colspan="4"><button type="submit" class="btn bc1 bs4">질문 등록</button>
			</tr>
		</form>
	</div>
	<%@include file="/WEB-INF/views/common/footer.jsp" %>
</body>
</html>